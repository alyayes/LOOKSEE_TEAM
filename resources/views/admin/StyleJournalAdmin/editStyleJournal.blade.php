        ...
        <form action="{{ route('stylejournalAdmin.update', $journal['id_journal']) }}" method="POST"
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="publication_date" name="publication_date"
                    value="{{ old('publication_date', date('Y-m-d', strtotime($journal['publication_date'] ?? date('Y-m-d')))) }}" required>
            </div>
            ...
            @if ($journal['image'])
                <div class="mb-3">
                    <p>Gambar Saat Ini:</p>
                    <img src="{{ asset('assets/images/journal/' . $journal['image']) }}" class="preview-img" alt="Gambar Jurnal">
                </div>
            @endif
